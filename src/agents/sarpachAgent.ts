...
import { CommandSuggestion } from '../commandHandler';
import { VerificationAgent } from './pluginAgents/verificationAgent';
import { DocumentationAgent } from './pluginAgents/documentationAgent';

interface IntentAnalysis {
    type: 'edit' | 'create' | 'tree' | 'analyze' | 'unknown' | 'document';
    command?: string;
    files: string[];
    instruction: string;
    confidence: number;
...
export class SarpachAgent {
    private readonly apiKey = 'AIzaSyBSu-Z0y_h1vSIoZEW6o8loDxp9V8Ylarg';
    private readonly modelName = 'gemini-2.0-flash';  // Ensure consistent model usage
...
MODIFIED CODE:`;
    ...
    private cleanResponse(response: string, fileType: string): string {
    ...
    private async handleFileContentRequest(input: string): Promise<{
        thinking: string;
        fileContent?: string;
        result: string;
    }> {
    ...
    private async handleFileModification(input: string): Promise<{
        thinking: string;
        command?: string;
        result: string;
        fileContent?: string;
    ...
    private generateCommand(intent: IntentAnalysis): string {
        switch (intent.type) {
            case 'edit':
    ...
    private async getDirectResponse(input: string): Promise<string> {
        ...
        try {
            ...
            if (command.startsWith('#create') || command.startsWith('#edit')) {
                ...
                for (const file of files) {
                    ...
                    const fileContent = await this.getLLMSuggestion(
                        file,
                        exists ? (await this.fileManager.getFileContent(file)) || '' : '',
                        instruction
                    );
    ...
    private async handleDocumentationCommand(): Promise<string> {
            ...
            async (progress, token) => {
                try {
                    ...
                    const outputPath = path.join(
                        outputDir,
                        `${projectName}_docs_${timestamp}.txt`
                    );
    ...
    private parseCommand(command: string): { files: string[], content: string, instruction: string } {
    ...
    private getFileTypeActions(ext: string): { command: string; description: string }[] {
        const commonActions = [
    ...
    async applyFileEdit(filePath: string, newContent: Promise<string>): Promise<boolean> {
        try {
    ...
    private async basicFileCheck(filePaths: string[]): Promise<{ success: boolean; details: string }> {
