...
import { FileSystemManager, FileTreeNode } from '../../fileManager';
...
interface DocumentSectionMetadata {
    complexity: number;
    dependencies: string[];
    codeQuality: {
...
export class DocumentationAgent {
    private readonly excludedDirs = ['node_modules', '.git', 'dist', 'build'];
    private readonly supportedExtensions = ['.ts', '.js', '.tsx', '.jsx', '.go', '.py', '.java', '.cpp', '.cs'];
    sideEffectsCounter: any;
    private readonly maxTokenSize = 4000;
    private readonly maxFileSizeKB = 100; // 100KB limit per file
    ...
    private async saveDocumentationWithBackup(outputPath: string, content: string): Promise<void> {
        try {
    ...
    private async generateEnhancedHeader(): Promise<string> {
...
`;
    ...
    private async generateFileStructureOverview(): Promise<string> {
        try {
    ...
    private async buildStructureOverview(): Promise<string[]> {
    ...
    private async getDirectoryContent(dirPath: string, level: number): Promise<string[]> {
        ...
        try {
            ...
            const significantFiles = sortedEntries.filter(entry => 
                this.isSignificantFile(entry)
            );
            ...
            if (level < 3) {
                ...
                for (const subdir of subdirs) {
                    const subdirContent = await this.getDirectoryContent(
                        path.join(dirPath, subdir.entry),
                        level + 1
                    );
    ...
    private async generateProjectOverview(tree: FileTreeNode): Promise<string> {
        const overview = `
Project Overview
---------------
...
${this.generateKeyFilesOverview(tree)}`;
    ...
    private formatDirectoryTree(node: FileTreeNode, indent: string): string {
    ...
    private async generateEnhancedFileDocumentation(files: FileContent[]): Promise<string> {
        const sections: string[] = [];
        const metadata = new Map<string, DocumentSectionMetadata>();
    ...
    private generateCrossReferenceAnalysis(metadata: Map<string, DocumentSectionMetadata>): string {
        // Only show significant relationships
        const significantDeps = new Map<string, string[]>();
        
        for (const [file, meta] of metadata.entries()) {
            const deps = meta.dependencies.filter(dep => 
                Array.from(metadata.keys()).some(f => f.includes(dep))
            );
            if (deps.length > 0) {
        ...
        if (significantDeps.size === 0) {return '';}
    ...
    private formatFileAnalysis(analysis: string, fileMetadata: DocumentSectionMetadata): string {
...
`;
    ...
    private async analyzeFileWithMetadata(file: FileContent): Promise<[string, DocumentSectionMetadata]> {
        const analysis = await this.analyzeFile(file);
        const metadata = await this.calculateFileMetadata(file);
        return [analysis, metadata];
    }

    private async calculateFileMetadata(file: FileContent): Promise<DocumentSectionMetadata> {
    ...
    private assessCodeQuality(content: string): { maintainability: number; testability: number; reusability: number; } {
    ...
    private async generateCodeMetrics(files: FileContent[]): Promise<string> {
    ...
    private getQualityRating(quality: { maintainability: number; testability: number; reusability: number }): string {
    ...
    private async getGitInfo(): Promise<{ version?: string; branch?: string; lastCommit?: string }> {
        try {
    ...
    private async generateFileDocumentation(files: FileContent[]): Promise<string> {
        const sections: string[] = [];
    ...
    private async analyzeFile(file: FileContent): Promise<string> {
...
[Analysis failed - skipping file]`;
    ...
    private async analyzeFileInChunks(file: FileContent): Promise<string> {
...
${analyses.join('\n\nContinued...\n\n')}`;
    ...
    private async extractProjectFiles(): Promise<FileContent[]> {
    ...
    private async getPackageInfo(): Promise<{ mainFiles: string[] }> {
        try {
    ...
    private sortFilesByImportance(files: string[], mainFiles: string[]): string[] {
    ...
    private calculateTestability(lines: string[]): number {
        let score = 100;
    ...
    private calculateReusability(lines: string[]): number {
        let score = 100;
    ...
    private getFunctionLengths(lines: string[]): number[] {
    ...
    private getMaxNestingLevel(lines: string[]): number {
        let maxNesting = 0;
    ...
    private countSideEffects(lines: string[]): number {
        let sideEffects = 0;
    ...
    private generateKeyFilesOverview(tree: FileTreeNode): string {
    ...
    private async analyzeDirectoryStructure(tree: FileTreeNode): Promise<string> {
